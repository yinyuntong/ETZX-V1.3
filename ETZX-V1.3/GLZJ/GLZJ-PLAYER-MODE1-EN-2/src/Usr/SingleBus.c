...
//	TH0 = TIMER0_COUNT;
	...
	TR0 = 1;												//启动定时器
	//其它控制设置
	byBus0SendStopCount = 230;								//上电总线0禁止发送时间设置
	IT1 = 1;												//外部中断1为下降沿触发模式   			
	if(Bus0RecPin)
	{	//如果总线正常,开中断			
		EX1 = 1;
...
**********************************************************/
#pragma disable
void Bus0RecInt(void) interrupt 2
{ 
...
{
	EnableIAP(IAP_READ);
	if(IapReadByte(IAP_ADDR) == INIT_FLAG)
	{//数据已经初始化完毕了,读取数据参数
		ReadParameter();
	...
	{	//没有初始化,初始化参数
		...
		stLocalControl.stEepromCfgData.byVoicePlayRoom = 0x00;		//不播报房号
		SaveParameter();
	}
}
...
/**********************************************************
*函数名称			:MakeCallVoiceData	
*函数描述        	:根据信息指示的内容构建语音报号的内容
*输入参数   		:pstIndicationData:信息指示指针
*返回值				:
*全局变量			:stLocalControl
*调用模块  			:
***********************************************************
...
*注释		      	:
**********************************************************/
void MakeCallVoiceData(pSTBusFreq pstBusDealFreq)
{	
	uint8 byPosition = 0;
	uint8 byTemp1;
	uint8 byTemp2;
...
//	Send_Data(&(pstBusDealFreq->bySndSecAddr),7);

	if(pstBusDealFreq->bySndRoomAddr == ADD_FJ_ADDR)
	{//是加床分机
		if((pstBusDealFreq->byRecSecAddr& 0x1f) == CMD_INFUSION_CALL)
		{//普通呼叫
			stLocalControl.stVoicePlay.byVoiceData[byPosition++] = EXTRA_VOICE; //"加"

		}
		else return;	//加床分机只有普通呼叫
	}
	...
	switch(pstBusDealFreq->byRecSecAddr& 0x1f)
	{
		case CMD_INFUSION_CALL: 		//输液呼叫命令
			stLocalControl.stVoicePlay.byVoiceData[byPosition++] = BED_VOICE;	//"bed"
			stLocalControl.stVoicePlay.byVoiceData[byPosition++] = EXTENSION_VOICE; //"extension"
			stLocalControl.stVoicePlay.byVoiceData[byPosition++] = NUMBER_VOICE;	//"number"
			...
			{	//有百位数据
				...
				byTemp2 = byTemp1 / 10;
				byTemp1 %= 10;
				stLocalControl.stVoicePlay.byVoiceData[byPosition++]=byTemp2;	//十位数据
				stLocalControl.stVoicePlay.byVoiceData[byPosition++]=byTemp1;	//个位数据
			}
			else 
			{//没有百位数据
				byTemp2 = byTemp1 / 10;
		...
		case CMD_SERVICE_CALL:			//服务呼收实质为输液完毕呼叫命令
			...
			stLocalControl.stVoicePlay.byVoiceSize = byPosition;			
			break;		
		case CMD_EMERGENCY_CALL:			//卫生间紧急呼叫命令
			stLocalControl.stVoicePlay.byVoiceData[byPosition++] = BATH_VOICE;	//"bath"
			stLocalControl.stVoicePlay.byVoiceData[byPosition++] = EXTENSION_VOICE; //"extension"
			stLocalControl.stVoicePlay.byVoiceData[byPosition++] = NUMBER_VOICE;	//"number"

			byTemp1 = pstBusDealFreq->bySndRoomAddr;
			byTemp2 = byTemp1 / 100;
			byTemp1 %= 100;
			if(byTemp2)
			{	//有百位数据
...
{
	stLocalControl.stVoicePlay.byVS1003State &= ~ISD_TIME_OVER;
	stLocalControl.stVoicePlay.byVS1003State |= ISD_REPLAY;
	stLocalControl.stVoicePlay.byVoiceTimes = 4;	// 1轮报号4遍
}
...
void Bus0RecDeal(void)
{
	...
	Send_Data(&(stLocalControl.stBusDealFreq.bySndSecAddr),7); 	//将接收到的数据发给PC机
	
	switch(stLocalControl.stBusDealFreq.byCmd)
	{
		case CMD_INFO_INDICATION:										//消息指示命令
			MakeCallVoiceData(&(stLocalControl.stBusDealFreq));
			//播放语音
			if(stLocalControl.stVoicePlay.byVoiceSize)
			{
/*				for(i=0;i<30;i++)
				{
					if(stLocalControl.stVoicePlay.byVoiceTimes==0) break;	//已报完,马上退出
		...
		case CMD_VOICE_PLAY_ROOM:
			...
			{
				stLocalControl.stEepromCfgData.byVoicePlayRoom =0x01;
			}
			else stLocalControl.stEepromCfgData.byVoicePlayRoom =0x00;
			SaveParameter();
			break;
