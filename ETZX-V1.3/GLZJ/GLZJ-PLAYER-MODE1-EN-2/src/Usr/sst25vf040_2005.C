...
unsigned char xdata FRAM_BUFF[32];
...
/*	FRAM_BUFF[0]=Read(0x080000);
	delay_nms(1);
	
	FRAM_BUFF[1]=Read(0x090000);

	delay_nms(1);	
	FRAM_BUFF[2]=Read(0x0a0000);
	delay_nms(1);
	Send_Data(FRAM_BUFF,3);*/
	...
	//SST25VF020   测试存储数据最多只到0x3ffff


	//最末一个扇区为0x3f000,0x3e000


	Sector_Erase(0x3e000);

	for(i=0;i<10;i++) Byte_Write1(0x3e000+i,i);

	for(i=0;i<10;i++) FRAM_BUFF[i]= 0;

	for(i=0;i<10;i++) FRAM_BUFF[i] = Read(0x3e000+i);

    Send_Data(FRAM_BUFF,10);
	 
			
	for(i=0x1000;i<0x2000;i++)
	{
		voice_data = Read(i);
		Send_Data_Byte(voice_data);
	}
