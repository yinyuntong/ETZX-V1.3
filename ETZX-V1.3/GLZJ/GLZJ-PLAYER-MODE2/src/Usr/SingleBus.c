...
bit bPlayVoiceMaintain=0;				//播放系统维护语音标志
bit bPlayVoiceWait=0;					//播放请稍候语音标志
...
{	
	...
	{//是加床分机
		...
		{//普通呼叫
			stLocalControl.stVoicePlay.byVoiceData[byPosition++] = ADD_VOICE;	//"加"
			...
			{	//有百位数据
				...
				stLocalControl.stVoicePlay.byVoiceData[byPosition++] = HUNDRED_VOICE;		//"百"音的序号
				if(byTemp1)
				{	//剩下的数据仍然有
					byTemp2 = byTemp1 / 10;
					byTemp1 %= 10;
					...
					if(byTemp2)
					{	//有十位数据,报十
						stLocalControl.stVoicePlay.byVoiceData[byPosition++] = TEN_VOICE;	//"十"音的序号
					}
					if(byTemp1)
					{	//有个位数据,报号
	...
	switch(pstBusDealFreq->byRecSecAddr& 0x1f)
	{
		case CMD_INFUSION_CALL:			//输液呼叫命令
			...
			stLocalControl.stVoicePlay.byVoiceSize = byPosition;			
			break;
		case CMD_SERVICE_CALL:			//实质为输液完毕呼叫命令
			byTemp1 = pstBusDealFreq->bySndBedAddr;
			byTemp2 = byTemp1 / 100;
			byTemp1 %= 100;
			if(byTemp2)
		...
		case CMD_EMERGENCY_CALL:			//卫生间紧急呼叫命令
			...
			if(byTemp1 == 0x00)
			{	//该房间只有一个卫生间，直接报卫生间呼叫
				stLocalControl.stVoicePlay.byVoiceData[byPosition++] = TOILET_VOICE;			//卫生间
				//呼叫
				stLocalControl.stVoicePlay.byVoiceData[byPosition++] = CALL_VOICE;	
				stLocalControl.stVoicePlay.byVoiceSize = byPosition;				
				break;
...
#endif	
//Send_Data(stLocalControl.stVoicePlay.byVoiceData,byPosition);
}
...
/*				for(i=0;i<30;i++)
		...
		case CMD_VOICE_MAINTAIN:
			if(stLocalControl.stBusDealFreq.bySndSecAddr ==0x01)	//开始播放系统维护语音
			{
				stLocalControl.stVoicePlay.byVoiceData[0] = MAINTAIN_VOICE;	
				stLocalControl.stVoicePlay.byVoiceSize = 1;	
				stLocalControl.stVoicePlay.byVS1003State &= ~ISD_TIME_OVER;
				stLocalControl.stVoicePlay.byVS1003State |= ISD_REPLAY;
				stLocalControl.stVoicePlay.byVoiceTimes = 1;	// 1轮报号1遍
				bPlayVoiceMaintain =1;

			}
			break;
        case CMD_VOICE_WAITE:
			if(stLocalControl.stBusDealFreq.bySndSecAddr ==0x01)	//开始播放系统维护语音
			{
				stLocalControl.stVoicePlay.byVoiceData[0] = WAIT_VOICE;	
				stLocalControl.stVoicePlay.byVoiceSize = 1;	
				stLocalControl.stVoicePlay.byVS1003State &= ~ISD_TIME_OVER;
				stLocalControl.stVoicePlay.byVS1003State |= ISD_REPLAY;
				stLocalControl.stVoicePlay.byVoiceTimes = 1;	// 1轮报号1遍
