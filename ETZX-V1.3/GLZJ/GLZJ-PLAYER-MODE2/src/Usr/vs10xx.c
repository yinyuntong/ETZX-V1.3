...
//  Mp3WriteRegister(0x03,0xf8,0x00);   //时钟设置，3号寄存器写0x9800，
...
//  delay_nms(1); // One millisecond delay 
//  delay_nus(10);
	_nop_();
	_nop_();
...
//  Mp3WriteRegister(SPI_CLOCKF, 0xf8, 0x00);
//  delay_nms(1); // One millisecond delay
//delay_nus(10);
  _nop_();
  _nop_();
...
{
	Mp3WriteRegister(0x0b,0xff,0xff);	//进入模拟掉电状态
	while(!MP3_DREQ);
	MP3_XCS =1;
	MP3_XDCS =1;
