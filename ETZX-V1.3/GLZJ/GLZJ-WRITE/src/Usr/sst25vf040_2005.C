...
unsigned char xdata FRAM_BUFF[100];
...
void WREN_Check()
{
	...
	if ((byte & 0x02)!= 0x02)				/* verify that WEL bit is set */
	{
		while(1) ;
...
//	delay_nms(25);
	...
	Send_Byte(((Dst & 0xFFFF) >> 8));
	Send_Byte(Dst & 0xFF);
	Send_Byte(*p);			//send 1st byte to be programmed 
	CE_High();				// disable device 
...
/*	 Byte_Write(0x000000,FRAM_BUFF[0]);
	 AAI_Write1(0x000002,&(FRAM_BUFF[1]),10);
	 
	 for(i=0;i<20;i++) FRAM_BUFF[i]=0;
	 delay_nms(10);

	 FRAM_BUFF[0]=Read(0x000000);
	 Read_Cont(0x000002,10,&(FRAM_BUFF[1]));
	 delay_nms(10);
	 Send_Data(FRAM_BUFF,11);
*/	 


	//SST25VF032   测试存储数据最多只到0x7fff0
	 AAI_Write1(0x7fff1,&(FRAM_BUFF[0]),11);
	 
	 for(i=0;i<20;i++) FRAM_BUFF[i]=0;
	 delay_nms(10);

	 Read_Cont(0x7fff1,11,&(FRAM_BUFF[0]));
	 delay_nms(10);
	 Send_Data(FRAM_BUFF,11);

/*	Byte_Write(0x7fff0,0x11);
	delay_nms(1);
	Send_Data_Byte(Read(0x7fff0));	

	Byte_Write(0x7fffe,0x11);
	delay_nms(1);
	Send_Data_Byte(Read(0x7fffe));	

	Byte_Write(0x100000,0x22);
	delay_nms(1);
	Send_Data_Byte(Read(0x100000));	


	Byte_Write(0x300000,0xbb);
	delay_nms(1);
	Send_Data_Byte(Read(0x300000));*/

/*	 Read_Cont(0x000002,20,FRAM_BUFF);	 
	 Send_Data(FRAM_BUFF,20);
*/
}
