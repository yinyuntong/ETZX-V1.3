...
UartRecError:
	...
	if(pstUartFreq->stLcdCtlData.byEndFlag ==0x00) 
	{
		...
		{//红色数据
			...
			memset((void *)(SystemInfo.DisplayInfo.MessageBuf),0x00,sizeof(SystemInfo.DisplayInfo.MessageBuf));
...
UartRecOk:	
    ...
    else if(pstUartFreq->stLcdCtlData.byEndFlag ==0x01) 
	{
		if(pstUartFreq->byData[0] !=0x20)
		{//绿色数据
			memset((INT8U *)(SystemInfo.DisplayInfo.MessageBuf),0x00,sizeof(SystemInfo.DisplayInfo.MessageBuf));
			...
			//eepromWrite((INT8U *)&SystemInfo); //保存参数:包括颜色数据
			Write_Cont(SYS_INFO_ADD,(INT8U*)&SystemInfo,sizeof(SystemInfo));
			goto UartRecOk;
	...
	{
	   	memset((void*)SystemInfo.DisplayInfo.MessageBuf,0,254);	
//		eepromWrite((INT8U*)&SystemInfo); //保存参数 
		Write_Cont(SYS_INFO_ADD,(INT8U*)&SystemInfo,sizeof(SystemInfo));
