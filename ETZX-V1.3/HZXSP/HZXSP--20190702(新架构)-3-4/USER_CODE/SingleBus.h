...
#define SD_HIGH_STATE  (GetBus0RecPin()==0)
#define SD_LOW_STATE	(GetBus0RecPin())

#define ACT_SD_HIGH	   (ClrBus0SendPin())
#define ACT_SD_LOW	   (SetBus0SendPin())

//隔离控制引脚定义
#define ProtectPin (1<<8)  //P2.8
#define SetProtectPin() (LPC_GPIO2->DATA|=ProtectPin)		//继电器动作，使屏与SD总线脱离
#define ClrProtectPin() (LPC_GPIO2->DATA&=~ProtectPin)		//继电器不动作，屏连上总线


#define SD_ON (ClrProtectPin())
#define SD_OFF (SetProtectPin())



extern INT8U TimerOutCount;			//20ms单元定时
extern INT8U TimerOutDealFlag;		//TimerOutDeal()函数处理标志
extern INT8U BusLowDTime;
...
extern INT8U BcdToHex(INT8U byData);
