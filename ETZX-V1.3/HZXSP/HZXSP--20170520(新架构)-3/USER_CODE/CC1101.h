...
typedef struct
{
	...
	INT8U 	bySecAddr;										//区号
	INT8U	byFrameID;	 								    //数据帧号
	INT8U   null;											//暂未用 
	INT8U	byCmd;											//命令代码
	INT8U	byLen;										    //数据长度
	INT8U	byData[1];										//链路数据
} STC1101Freq, *pSTC1101Freq;
...
#define  READ_REG      0x80      //==CCxxx0_READ_SINGLE 
#define  READ_STATUS   0xC0      //==CCxxx0_READ_BURST



#define cc1101RecPin (1<<2)	//P1.2									//CC1101数据接收中断管脚
#define Getcc1101RecPin() (LPC_GPIO1->DATA&cc1101RecPin)

//************************************************
#define LENGTH   32    //数据长度最大值
#define ADDR_VAL1   0x01   //自己的地址（高位--簇号）
#define ADDR_VAL0   0x02   //自己的地址（低位--分节点号）
#define PA_VALE     0xc0    // OUT +10dBm
//==============================================
...
extern void halSpiStrobe(INT8U strobe);
extern void halRfWriteRfSettings(void);
extern void cc1101Hander(void);
extern void cc1101Pin_init(void);
extern void CC1101_Init(void);


extern INT8U bWireLessRec;	
extern INT8U byWireLessRecLen;	//数据长度
extern INT8U byWireLessRecBuff[];
extern INT16S DataLenTotal;			//  1次广播信息的总长度
extern INT8U RecedDataLen;				//已接收的数据长度
extern INT8U byRecError;				//接收出错
extern INT8U byRecStarted;			//已收了第一帧
extern INT8U RecFrameID;
extern INT8U ScreenNum;
