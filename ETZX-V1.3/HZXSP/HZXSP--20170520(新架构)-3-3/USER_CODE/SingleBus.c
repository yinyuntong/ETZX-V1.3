...
INT8U TimerOutCount=0;			//20ms单元定时
INT8U TimerOutDealFlag=0;		//TimerOutDeal()函数处理标志
INT8U BusLowDTime=100;

INT8U bBus0SndBitOk =0;
//----------------------------------
		...
		if(KeyDownDelay)
		{
			...
			if(KeyDownDelay==0)
			{
				...
				if(GetKeyValue()==0)
				{
					...
					KeyDownDelay  =  KEY_DOWN_DELAY;	//由已经按下状态到　是否长按状态下的一个时间变量
														//20ms*50=1S
				...
				if(GetKeyValue()==0)
				{
					...
					if(KeyAlwaysDownDelay)
					{
						...
						if(KeyAlwaysDownDelay==0)
						{
							...
							KeyAlwaysDownDelay = 25;	//　0.5秒间隔, 产生一个长按处理
							KeyState |= KEY_ALWAYS_DOWN_HANDLER;
...
void Bus0RecDeal(void)
{
	INT8U temp;
/*	INT8U *P;
	P =(INT8U *) (&(stBusDealFreq.bySndSecAddr));
	for(temp=0;temp<7;temp++)
	{
		uartSendByte(*P);
		P++;
...
/*			SystemInfo.DisplayInfo.CallBuf[0] =  stBusDealFreq.byRecSecAddr&0x1f;  //呼叫类型
			...
			//uartSendByte(CMD_INFO_INDICATION);
			break;
		...
		case CMD_EMERGENCY_ANSWER:
		case CMD_HELP_ANSWER:
			SystemInfo.DisplayInfo.CallBuf[0] =  stBusDealFreq.byCmd - 0x06;  //呼叫类型
			SystemInfo.DisplayInfo.CallBuf[1] =  stBusDealFreq.byRecSecAddr;		//区号
			SystemInfo.DisplayInfo.CallBuf[2] =  stBusDealFreq.byRecRoomAddr;		//房号
			SystemInfo.DisplayInfo.CallBuf[3] =  stBusDealFreq.byRecBedAddr;		//床号
			
			if(PreCallInfoOnClr(SystemInfo.DisplayInfo.CallMessageBuf) ==2)  
			{
				SystemInfo.DisplayInfo.state&=~DISPLAY_STATE_CALL;
				SystemInfo.DisplayInfo.state |= DISPLAY_STATE_CHANGED;			
			}
		...
		case CMD_EMERGENCY_CLEAR:
		case CMD_HELP_CLEAR:
			SystemInfo.DisplayInfo.CallBuf[0] =  stBusDealFreq.byCmd - 0x0a;  //呼叫类型
			SystemInfo.DisplayInfo.CallBuf[1] =  stBusDealFreq.byRecSecAddr;		//区号
			SystemInfo.DisplayInfo.CallBuf[2] =  stBusDealFreq.byRecRoomAddr;		//房号
			SystemInfo.DisplayInfo.CallBuf[3] =  stBusDealFreq.byRecBedAddr;		//床号
		...
		case CMD_HELP_ENTER:
			SystemInfo.DisplayInfo.CallBuf[0] =  stBusDealFreq.byCmd - 0x1b;  //呼叫类型
			SystemInfo.DisplayInfo.CallBuf[1] =  stBusDealFreq.byRecSecAddr;		//区号
			SystemInfo.DisplayInfo.CallBuf[2] =  stBusDealFreq.byRecRoomAddr;		//房号
			SystemInfo.DisplayInfo.CallBuf[3] =  stBusDealFreq.byRecBedAddr;		//床号

			memset((INT8U *)(SystemInfo.DisplayInfo.CallMessageBuf),0x00,sizeof(SystemInfo.DisplayInfo.CallMessageBuf));
			
			//memcpy((INT8U *)(SystemInfo.DisplayInfo.CallMessageBuf),"恭祝大家2019年猪年新春快乐，猪年大发!",38);	
			PreCallInfoOnCall((INT8U *)(SystemInfo.DisplayInfo.CallMessageBuf));
		...
		case CMD_POWER_ON:	  								//本机重新热启动
			...
			//}
			break;	
		case CMD_SYSTEM_SEC_SET:		//系统区号设置命令
			SystemInfo.LOCAL_ADDR = stBusDealFreq.byRecSecAddr;
			SaveParameter(SYS_INFO_ADD);
			SaveParameter(SYS_INFO_ADD_BAK);
			break;
	}
...
{
	sprintf(buf,"%s","SD总线故障");
	
	Num = GetNextDisplayFrame();
	SetOperateFrame(Num);
	ClsScreen(YELLOW);
	TextOutRed(8,0,buf,FONT_DEFAULT,16);
	SetDisplayFrame(Num); 
}
