...
#define 	DisableBus0RecInt()	{EX0 = 0; MUT=OFF;} 			//如果在使用单总线1，不关闭喇叭    单总线0禁止总线中断宏定义
...
#define BUS1_TEST_TIME 3000	//大概1分钟

#define OX_SUPPLY_STATE 0x04	//bit2
...
#define CMD_SB_DISPLAY_DATA		0x82	/*发送一帧液晶屏数据*/
...
#define CMD_SHORT_CHECK			0xa0	//上位机短路测试
#define CMD_VOLTAGE5_CHECK		0xa1	//上位机5V电压测试
#define CMD_VOLTAGE3_CHECK      0xa2	//上位机3.3V电压测试
#define CMD_VOLTAGE1_CHECK      0xa3	//上位机1.2V电压测试
#define CMD_BUS0_CHECK			0xa4	//单总线1测试命令
#define CMD_BUS1_CHECK			0xa5	//单总线1测试命令

#define CMD_SD_NOR_VOL_CHECK	0xa6	//常态下SD线上电压测试
#define CMD_SA_NOR_VOL_CHECK    0xa7	//常态下SA线上电压测试

#define CMD_SD_TAL_VOL_CHECK	0xa8	//通话情总下SD线上电压测试
#define CMD_SA_TAL_VOL_CHECK    0xa9	//通话情总下SA线上电压测试

#define CMD_SD_TAL_VOL_CHECK_END 0xaA	//通话情总下SD线上电压测试结束
#define CMD_SA_TAL_VOL_CHECK_END 0xaB	//通话情总下SA线上电压测试结束
/*****************************************************************/
...
extern uint8 Bus1OutputData(uint8* pbyData);
...
sbit	bMUTState			=	byDevState3^0;				//MUT控制脚状态
//许可标志定义
...
extern 	uint8 	bdata byEnable1;
extern	uint8 	bdata byEnable2;
extern	bit  	bBusy;
...
extern uint8 xdata byIrDARecBuff[]; 
extern uint8 xdata byIrDADealBuff[];
...
extern void IrDATreat(void);
extern void IrDANumberSet(void);
extern void ReadRFSerial(void);
extern void SaveRFSerial(void);
extern void SaveParameter(uint16 addr);
extern void BusManage(void);
extern uint8 CalcParaCrc(void);
