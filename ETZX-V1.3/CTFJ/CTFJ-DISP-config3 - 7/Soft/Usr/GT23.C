...
void Get_GT23_Data(uint8 *word,uint8 word_size);
		...
		uint32	xdata address1,address2,address3,address4;
		 ...
		 if(highbyte<0x7f&&highbyte>=0x20) //	0x21~0x7e  是ASCII字符
		 {
			 switch(word_size)
			 {
			  ...
			  case 4:	//96x48   //因GT23_REC_DATA存储空间有限，不能按此处理
				/*  if(highbyte<=0x39&&highbyte>=0x30)  //数字
				 {
				  Send_Data_Byte(0x97);
				  address1 = (highbyte-0x21)*576; 
