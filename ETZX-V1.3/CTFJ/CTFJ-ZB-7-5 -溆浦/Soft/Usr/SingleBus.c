...
//保存输液报警器序列号
/*void SaveRFSerial(void)
{  
	stLocalControl.uiShortType = RF_SERIAL_ADDR;	//EEPROM中起始地址	
	stLocalControl.pbyData = RFSerial;				//被保存的数据
	stLocalControl.byChar1 = 6;						//6个字节
	EnableIAP(IAP_ERASE);
	IapErase(IAP_ADDR); 	
	EnableIAP(IAP_WRITE); 
	while(stLocalControl.byChar1--)
...
*/	
		...
		case CMD_BED_VOICE_PLAY:
			...
			else stLocalControl.stEepromCfgData.byBedFjFlag &= ~BedVoicePlay;
			SaveParameter();
			break;
