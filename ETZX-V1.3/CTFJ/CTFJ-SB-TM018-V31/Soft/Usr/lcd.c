...
void ShowPowerUpFace(void)
{
	...
	x=(128-16*5)/2;
	y=(160-16*2)/2;
	DisplayChineseString(&x,&y,"系统初始化",5,WHITE,BLUE,1);



	x=(128-16*5)/2;
	y +=20;
	DisplayChineseString(&x,&y,"版本",2,WHITE,BLUE,1);
	DisplayASCIIString(&x,&y,"V",1,WHITE,BLUE,1);
...
void ShowLandingFace(void)
{
	...
	x=0;y=0;	
	DisplayDigitString(&x,&y,&temp1,1,RED,BLUE,3);
	DisplayDigitString(&x,&y,&temp2,1,RED,BLUE,3);
	DisplayChineseString(&x,&y,"房",1,RED,BLUE,3);
	
	temp1=stLocalControl.stEepromCfgData.bySelfBedAddr%100;
	temp2=temp1%10;	//个位
	temp1=temp1/10;	//十位
	DisplayDigitString(&x,&y,&temp1,1,RED,BLUE,3);
	DisplayDigitString(&x,&y,&temp2,1,RED,BLUE,3);
	DisplayChineseString(&x,&y,"床",1,RED,BLUE,3);	
	...
	DisplayBlock(0,96,31,127,GREEN_CODE);
	x=32;y=96;
	DisplayChineseString(&x,&y,"有需要",3,BLUE,GREEN,3);
//	DisplayBlock(x,y,127,127,PURPLE_CODE);
	
	
//	DisplayBlock(0,128,15,159,PURPLE_CODE);
	x=0;y=128;
	DisplayChineseString(&x,&y,"请呼叫",3,BLUE,GREEN,3);
	DisplayASCIIString(&x,&y,"!",1,BLUE,GREEN,3);
...
void ShowMainMenuFace(void)
{
	...
	if(MainMenuSelect==1)
	{//被选择
		DisplayASCIIString(&x,&y,"1.",2,BLUE,WHITE,2);
		DisplayChineseString(&x,&y,"饮食方式",4,BLUE,WHITE,2);
	}
	else
	{
		DisplayASCIIString(&x,&y,"1.",2,WHITE,BLUE,2);
		DisplayChineseString(&x,&y,"饮食方式",4,WHITE,BLUE,2);
	}	
	...
	if(MainMenuSelect==2)
	{//被选择
		DisplayASCIIString(&x,&y,"2.",2,BLUE,WHITE,2);
		DisplayChineseString(&x,&y,"医嘱",2,BLUE,WHITE,2);
	}
	else 
	{
		DisplayASCIIString(&x,&y,"2.",2,WHITE,BLUE,2);
		DisplayChineseString(&x,&y,"医嘱",2,WHITE,BLUE,2);
	}
	...
	if(MainMenuSelect==3)
	{//被选择
		DisplayASCIIString(&x,&y,"3.",2,BLUE,WHITE,2);
		DisplayChineseString(&x,&y,"提示消息",4,BLUE,WHITE,2);
	}
	else 
	{//被选择
		DisplayASCIIString(&x,&y,"3.",2,WHITE,BLUE,2);
		DisplayChineseString(&x,&y,"提示消息",4,WHITE,BLUE,2);
	}
...
{ 
	...
	DisplayChineseString(&x,&y,"通话中",3,WHITE,BLUE,3);
...
{ 
	...
	DisplayChineseString(&x,&y,"广播中",3,WHITE,BLUE,3);
...
void ShowCallSuccess(void)
{
	...
	DisplayChineseString(&x,&y,"呼叫成功",4,WHITE,BLUE,3);

	x=(128-32*3)/2;
	y+=16;	
	DisplayChineseString(&x,&y,"请等待",3,WHITE,BLUE,3);
...
//整个信息的数据长度
	...
	if(TotalDataLen == 0)	//没有数据
	{
		Lcd_Clear(BLUE);	//清屏
		x=0;y=64;
		DisplayChineseString(&x,&y,"没有数据",4,WHITE,BLUE,3);
		//return;
