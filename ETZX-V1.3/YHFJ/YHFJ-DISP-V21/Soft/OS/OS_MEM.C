...
//static struct _FreeMem xdata * xdata OSFreeMem;                /* 自由内存块的双向链表的头 */
...
void UserMemChkErr(void xdata *Addr) 
{
    Addr = Addr;
    while (1);
}
...
void xdata *OSMemNew(unsigned int Size) 
{
    struct _FreeMem xdata * xdata ThisFreeMem;
    struct _UsingMem xdata * xdata Rt;
...
void OSMemFree(void xdata *Addr) 

{
    struct _FreeMem xdata * xdata ThisFreeMem;
    struct _UsingMem xdata * xdata UserMem;
    unsigned int Size;
